...
public class Main
{
    ...
    private void run(boolean runningLocally)
    {
        ...
        int runTestCases = inUtils.readOneIntLine();
        for (int i = 0; i < runTestCases; i++)
        {
            int[] rowsColumns = inUtils.getNextNDigitFromLine(2);
            ...
            int sonarNeededForRows = rows / 3;
            int sonarNeededForColumns = columns / 3;
            ...
            if (columns % 3 != 0)
            {
                sonarNeededForColumns++;
            }
            int totalSonar = sonarNeededForColumns * sonarNeededForRows;
            resLines += totalSonar +"\n";
        }
