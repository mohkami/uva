...
public class Main
{
    ...
    private void run(boolean runningLocally)
    {
        ...
        {
            int students = inUtils.readNextInt();
            int[] studentsPaces = inUtils.getNextNDigitFromLine(students);
            Arrays.sort(studentsPaces);
            int maxSpeed = studentsPaces[students - 1];
            resLines += "Case " + (i + 1) + ": " + maxSpeed + "\n";
        }
