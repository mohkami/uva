...
public class Main
{
    ...
    private void run(boolean runningLocally)
    {
        ...
        String resLines = "";
        String lineValue = inUtils.readLine();
        while (!(lineValue.length() == 1 && lineValue.charAt(0) == '0'))
        {
            lineValue = lineValue.trim();

            String g = calculateGValue(lineValue);
            resLines += g + "\n";
            lineValue = inUtils.readLine();
        }
...
//            outUtils.printLine(lines + "\n --------- ");
    ...
    private String calculateGValue(String line)
    {
        if (line.length() == 1)
        {
            return line;
        ...
        return calculateGValue(String.valueOf(sum));
    }
